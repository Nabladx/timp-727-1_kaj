...
void reverse_round_node(struct node* n){

	if (n!=NULL) {

		if(n->left!=NULL && n->right!=NULL){}

          reverse_round_node(n->left);

          reverse_round_node(n->right);
...
void reverse_round_tree(struct tree* t){

	reverse_round_node(t->root->left);

	reverse_round_node(t->root->right);
